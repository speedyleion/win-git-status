...
fn index_has_one_entry() {
    ...
    common::test_repo(&temp, vec![Path::new("some_file.txt")]);
...
fn index_has_more_deeply_nested_entries() {
    ...
    assert_eq!(
        true,
        index.entries.len() == directories.len()
            && directories.iter().all(|k| index.entries.contains_key(*k))
    );
    assert_eq!(
        index.entries.get("dir_1/dir_2/dir_3").unwrap()[0].name,
        "file.txt"
    );
}
